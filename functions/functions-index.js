...
exports.onTaskCreated = onValueWritten(
  ...
  async (event) => {
    ...
    // Notifier l'assigné + l'auteur
    const assigneeTokens = assignee ? await getTokensForUser(assignee) : [];
    const authorTokens = await getTokensForUser(author);
    const allTokens = [...new Set([...assigneeTokens, ...authorTokens])];
